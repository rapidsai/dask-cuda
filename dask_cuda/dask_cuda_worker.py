...
def main(scheduler, host, worker_port, listen_address, contact_address,
    ...
    if nprocs > 1 and worker_port != 0:
        logger.error("Failed to launch worker.  You cannot use the --worker-port argument when nprocs > 1.")
        exit(1)

    if nprocs > 1 and nanny_port != 0:
        logger.error("Failed to launch worker.  You cannot use the --nanny_port argument when nprocs > 1.")
        exit(1)
